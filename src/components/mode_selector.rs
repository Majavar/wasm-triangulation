...
use dioxus_free_icons::{
    Icon,
    icons::go_icons::{GoDeviceDesktop, GoMoon, GoSun},
};
...
impl Mode {
    ...
    fn icon(&self) -> Element {
        match self {
            Mode::Light => rsx! { Icon { icon: GoSun, class: "h-6 w-6" }},
            Mode::Dark => rsx! { Icon { icon: GoMoon, class: "h-6 w-6" }},
            Mode::System => rsx! { Icon { icon: GoDeviceDesktop, class: "h-6 w-6" }},
        }
    }
